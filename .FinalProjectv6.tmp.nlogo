...
circle-operator-stack circle-operand-stack current-rule circle-turtle-stack operator-operand-link-stack
first-operation stack-length]
...
to choose-draw-operator-model [expression_item evaluation operandA operandB]
  ...
  [ draw-circle-or-square evaluation expression_item "square 2"]
end
...
if stack-string = "turtle-stack"[
    ...
    report popped-value]
if stack-string = "operator-operand-link-stack"[
    let popped-value first operator-operand-link-stack
...
to reset-variables
  ...
  set circle-operand-stack []
  set circle-operator-stack []
  set circle-turtle-stack []
  set operator-operand-link-stack []
...
to push-operator-operand-links
  let turtleA pop-stack "turtle-stack"
  let turtleB pop-stack "turtle-stack"
  push-stack (list turtleA turtleB) "operator-operand-link-stack"
end

to create-operand-operator-links [operator model-id]
  let list-of-operands pop-stack "operator-operand-link-stack"
  ask circle-model model-id [carefully [create-link-from circle (first list-of-operands)]
    [create-link-from breed-result (first list-of-operands)]]
  ask circle-model model-id [carefully [create-link-from circle (last list-of-operands)]
    [create-link-from breed-result (last list-of-operands)]]
end
...
;; I kill them because I redraw from scratch.
;; I want larger circles or squares to be behind smaller ones
to kill-turtles-in-stack [local-circle-stack]
  loop [
...
to draw [expression_item model-shape operandB]

    set stack-length stack-length + 1
    create-circle-models 1 [
    set shape model-shape
    set size stack-length * 3
    position-circle-turtle-based-on-precedence
    attach-banner (word expression_item " " operandB)
...
to loop-circle-stack [local-circle-stack local-operator-operand-link-stack model-shape]
    if empty? local-circle-stack [stop]
    let sstack-length length local-circle-stack
    let local-operand-id first local-operator-operand-link-stack
    if(length local-operator-operand-link-stack > 1)
    [ carefully [make-actual-circle-model model-shape local-circle-stack stack-length first local-operand-id][]
      carefully[make-actual-circle-model model-shape local-circle-stack stack-length last local-operand-id][]]

    set local-operator-operand-link-stack butfirst local-operator-operand-link-stack
    set local-circle-stack butfirst local-circle-stack
    loop-circle-stack local-circle-stack local-operator-operand-link-stack model-shape
end


to make-actual-circle-model [model-shape local-circle-stack sstack-length operand-id]
    create-circle-models 1 [
    set shape model-shape
    set size sstack-length * 3
    position-circle-turtle-based-on-precedence
    attach-banner first local-circle-stack
    let circle-size (size / 2)
    ask link-neighbors [if breed = banners [reposition 45 circle-size - 1 show circle-size]]
    set circle-turtle-stack sentence who circle-turtle-stack
    ;create-operand-operator-links  model-id
    set sstack-length sstack-length - 1]
end

;;start at different positions on the screen note min-pxcor and max-pxcor
to position-circle-turtle-based-on-precedence
  if-else current-rule = 1
