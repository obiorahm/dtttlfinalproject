globals [expressions postfix_expression stack evaluation-stack yincr currx curry expression-stack hide-operators operand-list turtle-tracker]
...
to specifyPrecedence

end


;;parse arithmetic expression entry to postfix form
to parseEntry
  let expression Input_Expression
  test_precedence expression
end


to test_precedence [expression]
...
to push-evaluation-stack [operand]
  set evaluation-stack sentence operand evaluation-stack
end

to-report pop-evaluation-stack
...
to-report specify-model
  if Model_chooser = "Circle" [report "circle"]
  if Model_chooser = "Square" [report  "square 2"]
  if Model_chooser = "Tree" [report "circle"]
  if Model_chooser = "leaf" [report "leaf"]
end
...
to operand-to-operator-links [operator]
  ask boperator operator [carefully [create-link-to circle first operand-list]
    [create-link-to breed-result first operand-list] ]
  ask boperator operator [carefully [create-link-to circle last operand-list]
    [create-link-to breed-result last operand-list] ]
end
